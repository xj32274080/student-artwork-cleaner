(function exposeGalleryExport() {
  ...
  function displayName(item) {
    return item.artworkTitle || item.name || '学生作品';
  }
  ...
  function buildGalleryHtml(items, imageNames, themeName, filterName, musicName, meta = {}) {
    const colors = themeColors(themeName);
    const filter = window.ImageCleaner.filterCss(filterName);
    const title = meta.exhibitionTitle || '学生美术作品展';
    const subtitle = [meta.schoolName, meta.className].filter(Boolean).join(' · ') || '童心入画 · 色彩成诗 · 每一幅都值得被认真看见';
    const cards = items.map((item, index) => {
      const note = authorLine(item);
      return `
      <figure class="card">
        <div class="photo-wrap"><img src="images/${encodePathPart(imageNames[index])}" alt="${escapeHtml(displayName(item))}" /></div>
        <figcaption>
    ...
    const audio = musicName
      ? `<div class="music"><button id="playBtn" type="button">播放 / 暂停音乐</button><audio id="bgm" src="music/${encodePathPart(musicName)}" loop controls></audio></div>`
      : '';
...
.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:26px;max-width:1500px;margin:0 auto;padding:32px}.card{position:relative;margin:0;padding:16px;border:1px solid var(--line);border-radius:8px;background:var(--card);box-shadow:0 18px 45px rgba(80,55,25,.13);transition:.22s transform,.22s box-shadow}.card:hover{transform:translateY(-4px);box-shadow:0 24px 58px rgba(80,55,25,.18)}.card:before{content:"";position:absolute;inset:9px;border:1px dashed var(--line);border-radius:6px;pointer-events:none;opacity:.75}.photo-wrap{display:flex;align-items:center;justify-content:center;height:300px;overflow:hidden;border:1px solid #f1e5d1;border-radius:6px;background:#fff}.photo-wrap img{max-width:100%;max-height:100%;object-fit:contain;filter:var(--filter);cursor:pointer}.card figcaption{text-align:center;padding:12px 4px 2px;color:var(--muted);font-size:15px}.card figcaption strong{display:block;color:var(--ink);font-size:16px}.card figcaption span{display:block;margin-top:5px}.lightbox{position:fixed;inset:0;display:none;align-items:center;justify-content:center;padding:24px;background:rgba(0,0,0,.84);z-index:99}.lightbox.show{display:flex}.lightbox img{max-width:96vw;max-height:90vh;border-radius:8px;background:#fff;filter:var(--filter)}.corner{position:fixed;width:90px;height:90px;pointer-events:none;opacity:.35}.c1{left:18px;top:18px}.c2{right:18px;top:18px;transform:rotate(90deg)}.c3{left:18px;bottom:18px;transform:rotate(-90deg)}.c4{right:18px;bottom:18px;transform:rotate(180deg)}.corner:before{content:var(--mark);font-size:74px;color:var(--accent)}
@media(max-width:640px){h1{font-size:30px}.grid{padding:18px;gap:18px}.photo-wrap{height:250px}}
</style>
</head>
<body>
<div class="corner c1"></div><div class="corner c2"></div><div class="corner c3"></div><div class="corner c4"></div>
<header><h1>${escapeHtml(title)}</h1><div class="sub">${escapeHtml(subtitle)}</div>${audio}</header>
...
<\/script>
  ...
  async function buildCleanZip(items) {
    ...
    for (let index = 0; index < items.length; index += 1) {
      ...
      zip.file(safeFileName(`${String(index + 1).padStart(2, '0')}_${displayName(items[index])}.jpg`), blob);
    }
  ...
  async function buildGalleryZip(items, options) {
    ...
    const imageNames = [];
    for (let index = 0; index < items.length; index += 1) {
      const name = safeFileName(`${String(index + 1).padStart(2, '0')}_${displayName(items[index])}.jpg`);
    ...
    zip.file('index.html', buildGalleryHtml(items, imageNames, options.themeName, options.filterName, musicName, options.meta));
    return zip.generateAsync({ type: 'blob' });
  }
