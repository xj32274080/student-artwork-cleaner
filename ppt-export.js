(function exposePptExport() {
  ...
  function addArtworkCard(slide, item, x, y, w, h, filterName, colors, pptx, big = false) {
    ...
    const captionHeight = big ? 0.32 : 0.26;
  ...
  async function downloadPptx(items, options) {
    ...
    slide.addText('学生美术作品展', { x: 1.2, y: 1.65, w: 11, h: 0.8, fontFace: 'Microsoft YaHei', fontSize: 34, bold: true, color: hex(colors.ink), align: 'center' });
    slide.addText('童心 · 色彩 · 想象力', { x: 1.2, y: 2.55, w: 11, h: 0.5, fontFace: 'Microsoft YaHei', fontSize: 18, color: hex(colors.muted), align: 'center' });
    ...
    for (let start = 0; start < items.length; start += 6) {
      ...
      items.slice(start, start + 6).forEach((item, index) => {
        ...
        addArtworkCard(slide, item, 0.65 + col * 4.15, 0.9 + row * 2.95, 3.45, 2.35, options.filterName, colors, pptx);
      });
    }
    ...
    items.forEach((item, index) => {
      slide = pptx.addSlide();
      decorateSlide(slide, pptx, colors);
      slide.addText(item.name || `作品 ${index + 1}`, { x: 0.8, y: 0.35, w: 11.7, h: 0.45, fontFace: 'Microsoft YaHei', fontSize: 20, bold: true, color: hex(colors.ink), align: 'center' });
      addArtworkCard(slide, item, 1.65, 1.05, 10, 5.7, options.filterName, colors, pptx, true);
    });

    await pptx.writeFile({ fileName: '学生美术作品展_展示PPT.pptx' });
  }
